...
## Project Knowledge
- **Database Engine:** PostgreSQL 12+ (Azure Database for PostgreSQL Flexible Server)
- **ORM / Query Builder:** N/A — direct SQL via psql and pgAdmin 4
- **Migration Tool:** pg_restore (restores SQL Server AdventureWorks 2016 converted to PostgreSQL format)
- **Migration Command:** `pg_restore -h <server> -U postgres -d adventureworks AdventureWorksPG.gz`
- **Database Connection:** `psql -h <server>.postgres.database.azure.com -U postgres -d adventureworks`
