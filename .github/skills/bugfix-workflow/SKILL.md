...
is reported — whether by a human, a failed restore, or the Triager agent. The workflow
prioritizes reproducing the bug before attempting a fix and requires a validation query to
confirm the fix and prevent recurrence.

In this database project, a "bug" may include: schema errors (wrong data types, missing
constraints), restore failures (`pg_restore` errors), incorrect or missing data in the
AdventureWorks schema, broken PowerShell scripts, or constraint violations that allow
invalid data.
...
| 4 | **Coder** | Writes failing validation query first, then fixes the schema/script, opens PR | Fix task, design guidance, repro steps | PR with fix and validation query, linked to bug report | Validation query fails before fix, passes after |
| 5 | **Tester** | Validates fix via psql, checks for regressions in related schema objects, verifies edge cases around the fix | PR, bug report, repro steps | Validation results, additional validation queries, defect reports | Original bug fixed; no regressions introduced |
...
| 7 | **Reviewer** | Reviews fix for correctness, minimal scope, validation query quality | PR, bug report, security assessment | Review decision, review comments | Fix is correct and minimal; PR approved |
| 8 | **Human** | Approves and merges the PR | Approved PR | Merged fix on target branch | Fix merged; changes verified via manual review and psql validation queries |
...
- Fix task issue with acceptance criteria (must include "validation query confirms fix")
...
**Coder → Tester**
- Open PR with the fix and a validation query, linked to the bug report issue
...
- Summary of validation queries run and their results
...
- The reported bug is fixed and verified by validation queries.
...
- **Validation query is mandatory**: The Coder must write a validation query that fails
  before the fix and passes after it. This is a hard requirement, not optional. The query
  proves the fix works and prevents the bug from recurring.
...
- **Security escalation**: If the Security Auditor determines the bug is exploitable (e.g.,
  data exposure, overly permissive access controls, credential leakage), escalate to the
  human immediately. The fix may need expedited
